...
def play1(nums, boards)
  nums.each do |num|
    break unless boards.each_with_index do |b, ind|
      if !b[:w] && move(b, num)
        # p "b#{ind}wins - #{num}"
        print "res1: ", calc_winner(b, num), "\n"
        break
      end
    end
  end
...
def play2(nums, boards)
  res = nil
  nums.each do |num|
    boards.each_with_index do |b, ind|
      if !b[:w] && move(b, num)
        # p "b#{ind}wins - #{num}"
        res = calc_winner(b, num)
      end
    end
  end
  res
